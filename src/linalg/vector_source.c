...
/**
 * Read a vector from a file and creates the corresponding PnlVect.
 * The data can be stored column or row wise.
 * Anything after a '#' is ignored up to the end of the line.
...
static BASE FUNCTION(, _unary_minus)(BASE a)
{
  return MINUS(ZERO, a);
}
...
static double FUNCTION(, _op_sqr_norm)(BASE a)
{
  return SQUARE_NORM(a);
}
static double FUNCTION(, _op_abs)(BASE a)
{
  return NORMONE(a);
}
      ...
      switch (type[i])
        ...
        case 'v' :
          ...
          if (size == -1) size = args[i].V->size;
            ...
            {
              PNL_CHECK(size != args[i].V->size , "incompatible size", "pnl_vect_find");
            }
          break;
        }
    }
...
static void FUNCTION(pnl_vect, qsort_aux)(TYPE(PnlVect) * m, PnlVectInt *t, int use_index, char order)
{
  ...
  switch (order)
    ...
    case 'd' :
      ...
      break;
    default:
      PNL_ERROR("unknow order", "pnl_vect_qsort");
      break;
    }
}
