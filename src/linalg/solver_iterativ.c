 ...
 * @return  int
 */
static int pnl_iteration_base_error_code(PnlIterationBase *it)
{
  return it->error;
}
 ...
 * @param it : a PnlIterationBase ptr.
 */
static void  pnl_iteration_base_begin(PnlIterationBase *it)
{
  it->iteration = 0;
}
 ...
 */
static void pnl_iteration_base_fail(PnlIterationBase *it, int err_code)
{
  it->error = err_code;
}
...
{
  (*it).error = err_code;
  PNL_ERROR(msg, "iterative solver");
}
