...
{
  PNL_CHECK(i < 0 || i > M->m - 1, "index out of range", "pnl_mat_set_row");
...
{
  PNL_CHECK(i < 0 || i > M->m - 1, "index out of range", "pnl_mat_set_row_from_ptr");
...
void FUNCTION(pnl_mat, swap_rows)(TYPE(PnlMat) *M, int i, int j)
{
  ...
  PNL_CHECK(i < 0 || i >= M->m, "first index is out of range", "FUNCTION(pnl_mat,swap_rows");
  PNL_CHECK(j < 0 || j >= M->m, "second index is out of range", "FUNCTION(pnl_mat,swap_rows");
  p = M->n % 3;
  for (k = 0; k < p; k++)
    {
      Mik = PNL_MGET(M, i, k);
...
{
  int i;
  PNL_CHECK(j < 0 || j >= M->n, "index of range", "mat_set_col");
...
{
  PNL_CHECK(i < 0 || i >= M->m, "index out of range", "pnl_mat_get_row");
...
{
  TYPE(PnlVect) V;
  PNL_CHECK(i < 0 || i >= M->m, "index out of range", "pnl_vect_wrap_mat_row");
...
{
  int i;
  PNL_CHECK(j < 0 || j >= M->n, "index out of range", "pnl_mat_get_col");
      ...
      switch (type[k])
        ...
        case 'm' :
          ...
          if (m == -1)
            ...
            {
              PNL_CHECK((m != args[k].V->m) || (n != args[k].V->n) ,
...
{
  int k, l;
  PNL_CHECK(i < 0 || len_i < 0 || i + len_i > M->m, "size exceeded", "pnl_mat_extract_subblock");
  PNL_CHECK(j < 0 || len_j < 0 || j + len_j > M->n, "size exceeded", "pnl_mat_extract_subblock");
...
{
  int k, l;
  PNL_CHECK(i < 0 || i + block->m > M->m, "size exceeded", "pnl_mat_set_subblock");
  PNL_CHECK(j < 0 || j + block->n > M->n, "size exceeded", "pnl_mat_set_subblock");
...
{
  int new_size;
  PNL_CHECK(i < 0 || i > M->m, "size exceeded", "pnl_mat_add_row");
...
{
  PNL_CHECK(i < 0 || i >= M->m, "size exceeded", "pnl_mat_del_row");
  if (i < M->m - 1)
    {
...
void FUNCTION(pnl_mat, max_index)(TYPE(PnlVect) *out, PnlVectInt *index, const TYPE(PnlMat) *A, char d)
{
  ...
  switch (d)
    ...
    case '*' :
      ...
      break;
    default :
      PNL_ERROR("unknow direction", "pnl_mat_max_index");
      break;
    }
...
void FUNCTION(pnl_mat, min_index)(TYPE(PnlVect) *out, PnlVectInt *index, const TYPE(PnlMat) *A,  char d)
{
  ...
  switch (d)
    ...
    case '*' :
      ...
      break;
    default :
      PNL_ERROR("unknow direction", "pnl_mat_min_index");
      break;
    }
  if (index != NULL) pnl_vect_int_resize(index, n);
  FUNCTION(pnl_vect, resize)(out, n);
...
void FUNCTION(pnl_mat, minmax_index)(TYPE(PnlVect) *out_min, TYPE(PnlVect) *out_max, PnlVectInt *index_min, PnlVectInt *index_max, const TYPE(PnlMat) *A, char d)
{
  ...
  switch (d)
    ...
    case '*' :
      ...
      break;
    default :
      PNL_ERROR("unknow direction", "pnl_mat_minmax_index");
      break;
    }
      ...
      if (use_index == PNL_TRUE)
        {
          for (i = 0; i < t->m; i++)
            for (j = 0; j < t->n; j++)
              {
                pnl_mat_int_set(t, i, j, j);
              }
        }
      break;
    default:
      PNL_ERROR("unknown direction", "pnl_mat_qsort_aux");
      break;
    }
  for (i = 0; i < loops; i++)
