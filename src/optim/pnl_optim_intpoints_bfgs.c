 ...
 * @param xinit: the initial value of x. Is should be strictly feasible. ie constraint(xinit)>0.
 * @return xinit_feasible: integer such that (xinit_feasible=0) if (c(xinit)>0), 1 otherwise.
 */
static int AllConstraints_Init(AllConstraints *all_constraints, PnlRnFuncRm* NL_Constraints, PnlVect* LowerBounds, PnlVect* UpperBounds, PnlVect* xinit)
{
 ...
 */
static void AllConstraints_Value(PnlVect *constraints_x, const PnlVect *x, AllConstraints *all_constraints)
{
    int i, index, nbr_var;
    nbr_var = x->size;
 ...
 *
 * @param grad_func: function computing the gradient. It may be NULL, in this case the gradient of objective function is estimated with finites differences.
 * @param func: function to be differentiated. It's callled in finites differences method when grad_func is not given.
...
****************************************************************************************/
int pnl_optim_intpoints_bfgs_solve(PnlRnFuncR * func, PnlRnFuncRm *grad_func, PnlRnFuncRm * nl_constraints, PnlVect* lower_bounds, PnlVect* upper_bounds, PnlVect * x_input, double tolerance, int iter_max, int print_algo_steps, PnlVect * x_output)
{
