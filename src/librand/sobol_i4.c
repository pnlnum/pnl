...
static int poly[PNL_SOBOL_I4_DIM_MAX2] =
{
...
void i4_sobol_init (PnlRng *rng, int dim)
{
  int i, j, j2, k, l, m;
  char includ[PNL_SOBOL_I4_LOG_MAX];
  int atmost;
  sobol_i4_state *state = (sobol_i4_state *) (rng->state);

  for ( i = 0; i < PNL_SOBOL_I4_DIM_MAX2; i++ )
    {
      for ( j = 0; j < PNL_SOBOL_I4_LOG_MAX; j++ )
        {
          state->v[i][j] = 0;
        }
    }
...
#include "sobol_pol.h"
  ...
  /*
   *  Set ATMOST = 2^PNL_SOBOL_I4_LOG_MAX - 1.
   */
  atmost = 0;
  for ( i = 1; i <= PNL_SOBOL_I4_LOG_MAX; i++ )
    {
      atmost = 2 * atmost + 1;
    }
  /*
   *  Find the highest 1 bit in ATMOST (should be PNL_SOBOL_I4_LOG_MAX).
   */
  state->maxcol = i4_bit_hi1 ( atmost );
  /*
 ...
 * 
 */
void I4_SOBOL ( PnlRng *rng, double quasi[ ] )
{

  int l, i;
  sobol_i4_state *state = (sobol_i4_state *) (rng->state);
  int *lastq = state->lastq;
  float recipd = state->recipd;
  int maxcol = state->maxcol;
  ...
  if ( rng->counter == 1 )
    {
       ...
       */
      l = i4_bit_lo0 (rng->counter -1);
      for ( i = 0; i < dim_num; i++ )
        {
          lastq[i] = (lastq[i] ^ state->v[i][l-1]);
        }
    }
  l = i4_bit_lo0 ( rng->counter );
  /*
