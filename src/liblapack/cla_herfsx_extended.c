...
static float c_b33 = 1.f;
 ...
 int cla_herfsx_extended__(int *prec_type__, char *uplo, 
	int *n, int *nrhs, complex *a, int *lda, complex *af, 
	int *ldaf, int *ipiv, int *colequ, float *c__, complex *b, 
	int *ldb, complex *y, int *ldy, float *berr_out__, int *
	n_norms__, float *err_bnds_norm__, float *err_bnds_comp__, complex *res,
	 float *ayb, complex *dy, complex *y_tail__, float *rcond, int *
	ithresh, float *rthresh, float *dz_ub__, int *ignore_cwise__, 
	int *info, unsigned long uplo_len)
{
    ...
    /* Local variables */
    float dxratmax, dzratmax;
    int i__, j;
    extern  int cla_heamv__(int *, int *, float *, 
	    complex *, int *, complex *, int *, float *, float *, 
	    int *);
    int incr_prec__;
    float prev_dz_z__, yk, final_dx_x__;
    extern  int cla_wwaddw__(int *, complex *, complex *, 
	    complex *);
    float final_dz_z__, prevnormdx;
    int cnt;
    float dyk, eps, incr_thresh__, dx_x__, dz_z__;
    extern  int cla_lin_berr__(int *, int *, int *
	    , complex *, float *, float *);
    float ymin;
    extern  int blas_chemv_x__(int *, int *, complex *
	    , complex *, int *, complex *, int *, complex *, complex *
	    , int *, int *);
    int y_prec_state__, uplo2;
    extern  int blas_chemv2_x__(int *, int *, complex 
	    *, complex *, int *, complex *, complex *, int *, complex 
	    *, complex *, int *, int *);
    extern int lsame_(char *, char *);
    extern  int chemv_(char *, int *, complex *, complex *
, int *, complex *, int *, complex *, complex *, int *
), ccopy_(int *, complex *, int *, complex *, 
	    int *);
    float dxrat, dzrat;
    ...
    extern  int chetrs_(char *, int *, int *, complex 
	    *, int *, int *, complex *, int *, int *);
    float normdx, hugeval;
...
/*         Calculate relative changes DX_X, DZ_Z and ratios DXRAT, DZRAT. */
		...
		yk = (r__1 = y[i__4].r, ABS(r__1)) + (r__2 = r_imag(&y[i__ + 
			j * y_dim1]), ABS(r__2));
		i__4 = i__;
		dyk = (r__1 = dy[i__4].r, ABS(r__1)) + (r__2 = r_imag(&dy[
			i__]), ABS(r__2));
		if (yk != 0.f) {
/* Computing MAX */
