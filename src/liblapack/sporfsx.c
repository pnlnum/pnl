...
static int c__1 = 1;
 ...
 int sporfsx_(char *uplo, char *equed, int *n, int *
	nrhs, float *a, int *lda, float *af, int *ldaf, float *s, float *
	b, int *ldb, float *x, int *ldx, float *rcond, float *berr, 
	int *n_err_bnds__, float *err_bnds_norm__, float *err_bnds_comp__, 
	int *nparams, float *params, float *work, int *iwork, int *
	info)
{
    ...
    extern  int xerbla_(char *, int *), spocon_(
	    char *, int *, float *, int *, float *, float *, float *, 
	    int *, int *);
...
/*     cwise error is at least CWISE_WRONG. */
	...
	for (j = 1; j <= i__1; ++j) {
		...
		rcond_tmp__ = sla_porcond__(uplo, n, &a[a_offset], lda, &af[
			...
			work[1], &iwork[1], (unsigned long)1);
	    } else {
		rcond_tmp__ = 0.f;
	    }
