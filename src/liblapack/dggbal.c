	...
	info)
{
    ...
    /* Builtin functions */
    double d_lg10(double *), d_sign(double *, double *), pow_di(
	    double *, int *);
...
/*  DGGBAL balances a pair of general float matrices (A,B).  This */
...
L350:
    sfmin = dlamch_("S");
    sfmax = 1. / sfmin;
    lsfmin = (int) (d_lg10(&sfmin) / basl + 1.);
    lsfmax = (int) (d_lg10(&sfmax) / basl);
    i__1 = *ihi;
    for (i__ = *ilo; i__ <= i__1; ++i__) {
	i__2 = *n - *ilo + 1;
...
/* Computing MAX */
	...
	ir = (int) (lscale[i__] + d_sign(&c_b71, &lscale[i__]));
/* Computing MIN */
...
/* Computing MAX */
	...
	jc = (int) (rscale[i__] + d_sign(&c_b71, &rscale[i__]));
/* Computing MIN */
