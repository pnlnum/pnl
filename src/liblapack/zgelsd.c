	...
	int *lwork, double *rwork, int *iwork, int *info)
{
    ...
    extern  int dlascl_(char *, int *, int *, 
	    ...
	    *, double *, double *, double *, int *), 
	    xerbla_(char *, int *), zgebrd_(int *, int *, 
...
), zlalsd_(char *, int *, int *, int *, double *, 
...
/*  ZGELSD computes the minimum-norm solution to a float linear least */
/*  squares problem: */
...
/* Computing MAX */
	    i__1 = (int) (log((double) minmn / (double) (smlsiz + 
		    1)) / log(2.)) + 1;
	    nlvl = MAX(i__1,0);
