...
static int c__1 = 1;
 ...
 int csprfs_(char *uplo, int *n, int *nrhs, complex *
...
, complex *, int *, complex *, complex *, int *);
    ...
    float lstres;
    extern  int csptrs_(char *, int *, int *, complex 
	    *, int *, complex *, int *, int *);
...
/* L40: */
		...
		i__3 = kk + k - 1;
		rwork[k] = rwork[k] + ((r__1 = ap[i__3].r, ABS(r__1)) + (
			r__2 = r_imag(&ap[kk + k - 1]), ABS(r__2))) * xk + s;
		kk += k;
/* L50: */
	    }
	} else {
