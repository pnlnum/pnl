...
static int c_n1 = -1;
static int c__5 = 5;
static float c_b14 = 0.f;
static int c__1 = 1;
static float c_b51 = -1.f;
static float c_b52 = 1.f;
	...
	iwork, int *info)
{
    ...
    /* Local variables */
    int i__, j, k, p, q, ie, je, mb, nb, is, js, pq;
    float dsum;
    int ppqq;
...
/*  respectively, with float entries. (A, D) and (B, E) must be in */
/*  generalized (float) Schur canonical form, i.e. A, B are upper quasi */
...
/*           Use unblocked Level 2 solver */
	    ...
	    if (dscale != 0.f) {
		if (*ijob == 1 || *ijob == 3) {
		    *dif = sqrt((float) ((*m << 1) * *n)) / (dscale * sqrt(
			    dsum));
		} else {
		    *dif = sqrt((float) pq) / (dscale * sqrt(dsum));
		}
	    }
