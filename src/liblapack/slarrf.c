...
static int c__1 = 1;
 ...
 int slarrf_(int *n, float *d__, float *l, float *ld, 
	int *clstrt, int *clend, float *w, float *wgap, float *werr, 
	...
	float *dplus, float *lplus, float *work, int *info)
{
    ...
    /* Local variables */
    int i__;
    float s, bestshift, smlgrowth, eps, tmp, max1, max2, rrr1, rrr2, znm2, 
	    growthbound, fail, fact, oldp;
    int indx;
    float prod;
    int ktry;
    float fail2, avgap, ldmax, rdmax;
    int shift;
...
/*     Compute the average gap length of the cluster */
    clwdth = (r__1 = w[*clend] - w[*clstrt], ABS(r__1)) + werr[*clend] + 
	    werr[*clstrt];
...
/*     Use a small fudge to make sure that we floatly shift to the outside */
...
/*     Initialize the record of the best representation found */
    ...
    fail2 = (float) (*n - 1) * mingap / (*spdiam * sqrt(eps));
    bestshift = lsigma;
