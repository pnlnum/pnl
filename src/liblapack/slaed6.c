	...
	float *d__, float *z__, float *finit, float *tau, int *info)
{
    ...
    float a, b, c__, f;
    int i__;
    float fc, df, ddf, lbd, eta, ubd, eps, base;
    int iter;
...
/* Computing MAX */
	r__1 = ABS(a), r__2 = ABS(b), r__1 = MAX(r__1,r__2), r__2 = ABS(
		c__);
	temp = MAX(r__1,r__2);
	a /= temp;
	b /= temp;
...
/*     others but recomputed at each call */
    ...
    eps = slamch_("Epsilon");
    base = slamch_("Base");
    i__1 = (int) (log(slamch_("SafMin")) / log(base) / 3.f);
