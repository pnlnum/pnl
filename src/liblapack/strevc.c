	...
	int *ldvr, int *mm, int *m, float *work, int *info)
{
    ...
    int i__, j, k;
    float x[4]	/* was [2][2] */;
    int j1, j2, n2, ii, ki, ip, is;
    float wi, wr, rec, ulp, beta, emax;
    int pair, allv;
    int ierr;
    float unfl, ovfl, smin;
...
/*  a float general matrix:  A = Q*T*Q**T, as computed by SHSEQR. */
...
/*          computed. */
/*          If w(j) is a float eigenvalue, the corresponding float */
...
/* Computing MAX */
			r__3 = (r__1 = work[j + *n], ABS(r__1)), r__4 = (
				r__2 = work[j + 1 + *n], ABS(r__2)), r__3 = 
				MAX(r__3,r__4);
			vmax = MAX(r__3,vmax);
			vcrit = bignum / vmax;
...
/* Computing MAX */
			r__3 = (r__1 = work[j + *n], ABS(r__1)), r__4 = (
				r__2 = work[j + n2], ABS(r__2)), r__3 = MAX(
				r__3,r__4);
			vmax = MAX(r__3,vmax);
			vcrit = bignum / vmax;
...
/*                    Scale if necessary */
			...
			work[j + 1 + n2] = x[3];
/* Computing MAX */
			r__1 = ABS(x[0]), r__2 = ABS(x[2]), r__1 = MAX(r__1,
				r__2), r__2 = ABS(x[1]), r__1 = MAX(r__1,
				r__2), r__2 = ABS(x[3]), r__1 = MAX(r__1,
				r__2);
			vmax = MAX(r__1,vmax);
			vcrit = bignum / vmax;
