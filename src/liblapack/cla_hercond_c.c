...
double cla_hercond_c__(char *uplo, int *n, complex *a, int *lda, 
	complex *af, int *ldaf, int *ipiv, float *c__, int *capply,
	 int *info, complex *work, float *rwork, unsigned long uplo_len)
{
    /* System generated locals */
    int a_dim1, a_offset, af_dim1, af_offset, i__1, i__2, i__3, i__4;
...
/*     Compute norm of op(A)*op2(C). */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		for (j = 1; j <= i__2; ++j) {
		    i__3 = j + i__ * a_dim1;
		    tmp += ((r__1 = a[i__3].r, ABS(r__1)) + (r__2 = r_imag(&
			    a[j + i__ * a_dim1]), ABS(r__2))) / c__[j];
		}
		i__2 = *n;
		for (j = i__ + 1; j <= i__2; ++j) {
		    i__3 = i__ + j * a_dim1;
		    tmp += ((r__1 = a[i__3].r, ABS(r__1)) + (r__2 = r_imag(&
			    a[i__ + j * a_dim1]), ABS(r__2))) / c__[j];
		}
	    } else {
		i__2 = i__;
		for (j = 1; j <= i__2; ++j) {
