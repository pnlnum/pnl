...
static float c_b35 = 1.f;
 ...
 int sgsvj1_(char *jobv, int *m, int *n, int *n1, 
	float *a, int *lda, float *d__, float *sva, int *mv, float *v, 
	int *ldv, float *eps, float *sfmin, float *tol, int *nsweep, 
	float *work, int *lwork, int *info)
{
    ...
    float t, rootsfmin, cs, sn;
    int jbc;
    float big;
    int kbl, igl, ibr, jgl, mvl, nblc;
    float aapp, aapq, aaqq;
    int nblr, ierr;
    extern double sdot_(int *, float *, int *, float *, int *);
    float aapp0, temp1;
    extern double snrm2_(int *, float *, int *);
    float large, apoaq, aqoap;
    extern int lsame_(char *, char *);
    float theta, small, fastr[5];
    int applv, rsvec;
...
, float *), xerbla_(char *, int *);
    ...
    extern  int slassq_(int *, float *, int *, float *, 
	    float *);
    float mxsinj;
    int emptsw, notrot, iswrot;
    float rootbig, rooteps;
    int rowskip;
    float roottol;
...
/*     Additional steering devices */
	...
	if (i__ > swband + 1 && mxaapq < (float) (*n) * *tol && (float) (*n) * 
		mxaapq * mxsinj < *tol) {
	    goto L1994;
	}
