...
#include "pnl/pnl_f2c.h"
 ...
 int cla_gbamv__(int *trans, int *m, int *n, 
	int *kl, int *ku, float *alpha, complex *ab, int *ldab, 
	complex *x, int *incx, float *beta, float *y, int *incy)
{
...
/* Computing MIN */
		...
		for (j = MAX(i__2,1); j <= i__3; ++j) {
		    ...
		    symb_zero__ = symb_zero__ && (x[i__2].r == 0.f && x[i__2]
			    .i == 0.f || temp == 0.f);
		    i__2 = j;
		    y[iy] += *alpha * ((r__1 = x[i__2].r, ABS(r__1)) + (r__2 
