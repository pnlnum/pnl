...
static int c__2 = 2;
 ...
 int dgesvj_(char *joba, char *jobu, char *jobv, int *m, 
	...
	int *info)
{
    ...
    extern  int dscal_(int *, double *, double *, 
	    int *);
    double scale, large, apoaq, aqoap;
    extern int lsame_(char *, char *);
    double theta, small, sfmin;
    int lsvec;
    ...
    extern  int daxpy_(int *, double *, double *, 
	    int *, double *, int *);
    int uctol;
    extern  int drotm_(int *, double *, int *, 
	    double *, int *, double *);
    int lower, upper, rotok;
    extern  int dgsvj0_(char *, int *, int *, 
	    double *, int *, double *, double *, int *, 
	    double *, int *, double *, double *, double *, 
	     int *, double *, int *, int *), dgsvj1_(
	    char *, int *, int *, int *, double *, int *, 
    ...
    extern  int dlaset_(char *, int *, int *, 
	    ...
	    xerbla_(char *, int *);
    int ijblsk, swband, blskip;
    double mxaapq;
    extern  int dlassq_(int *, double *, int *, 
	    double *, double *);
    double thsign, mxsinj;
    int emptsw, notrot, iswrot, lkahead;
    int goscale, noscale;
    double rootbig, epsilon, rooteps;
    int rowskip;
    double roottol;
...
/*  Purpose */
/*  ~~~~~~~ */
/*  DGESVJ computes the singular value decomposition (SVD) of a float */
...
/*     BIG         = ONE    / SFMIN */
    rootbig = 1. / rootsfmin;
    large = big / sqrt((double) (*m * *n));
    bigtheta = 1. / rooteps;

    tol = ctol * epsilon;
    roottol = sqrt(tol);

    if ((double) (*m) * epsilon >= 1.) {
	*info = -5;
	i__1 = -(*info);
	xerbla_("DGESVJ", &i__1);
	return 0;
...
/*     in A are detected, the procedure returns with INFO=-6. */
    ...
    scale = 1. / sqrt((double) (*m) * (double) (*n));
    noscale = TRUE;
    goscale = TRUE;
...
/* Computing MIN */
	d__1 = sn / aaqq, d__2 = big / (aapp * sqrt((double) (*n)));
	temp1 = MIN(d__1,d__2);
/*         AAQQ  = AAQQ*TEMP1 */
/*         AAPP  = AAPP*TEMP1 */
    } else if (aaqq >= sn && aapp >= temp1) {
...
/* Computing MIN */
	d__1 = sn / aaqq, d__2 = big / (sqrt((double) (*n)) * aapp);
	temp1 = MIN(d__1,d__2);
/*         AAQQ  = AAQQ*TEMP1 */
/*         AAPP  = AAPP*TEMP1 */
    } else {
...
/*     Additional steering devices */
	...
	if (i__ > swband + 1 && mxaapq < sqrt((double) (*n)) * tol && (
