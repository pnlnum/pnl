...
static int c__1 = 1;
 ...
 int ctrrfs_(char *uplo, char *trans, char *diag, int *n, 
	...
	complex *x, int *ldx, float *ferr, float *berr, complex *work, float 
	*rwork, int *info)
{
    /* System generated locals */
    int a_dim1, a_offset, b_dim1, b_offset, x_dim1, x_offset, i__1, i__2, 
	    i__3, i__4, i__5;
    ...
    extern  int ctrmv_(char *, char *, char *, int *, 
	    ...
	    int *, complex *, int *), clacn2_(
	    int *, complex *, complex *, float *, int *, int *);
		...
		if (nounit) {
			...
			for (i__ = 1; i__ <= i__3; ++i__) {
			    ...
			    s += ((r__1 = a[i__4].r, ABS(r__1)) + (r__2 = 
				     ...
				     * ((r__3 = x[i__5].r, ABS(r__3)) + (
				    r__4 = r_imag(&x[i__ + j * x_dim1]), ABS(
				    r__4)));
/* L110: */
			}
			rwork[k] += s;
