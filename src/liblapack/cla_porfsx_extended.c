...
static float c_b33 = 1.f;
 ...
 int cla_porfsx_extended__(int *prec_type__, char *uplo, 
	int *n, int *nrhs, complex *a, int *lda, complex *af, 
	int *ldaf, int *colequ, float *c__, complex *b, int *ldb, 
	complex *y, int *ldy, float *berr_out__, int *n_norms__, float *
	err_bnds_norm__, float *err_bnds_comp__, complex *res, float *ayb, 
	complex *dy, complex *y_tail__, float *rcond, int *ithresh, float *
	rthresh, float *dz_ub__, int *ignore_cwise__, int *info, 
	unsigned long uplo_len)
{
...
, int *, complex *, int *, complex *, complex *, int *
    ...
    float normdx;
    extern  int cpotrs_(char *, int *, int *, complex 
	    *, int *, complex *, int *, int *);
    float hugeval;
