	...
	float *ztemp, int *info)
{
    ...
    /* Local variables */
    int i__, k, mid, ptr, curr;
    extern  int srot_(int *, float *, int *, float *, 
	    int *, float *, float *);
    int bsiz1, bsiz2, psiz1, psiz2, zptr1;
...
/*     roots. */
    ...
    bsiz1 = (int) (sqrt((float) (qptr[curr + 1] - qptr[curr])) + .5f);
    bsiz2 = (int) (sqrt((float) (qptr[curr + 2] - qptr[curr + 1])) + .5f);
    i__1 = mid - bsiz1 - 1;
    for (k = 1; k <= i__1; ++k) {
	z__[k] = 0.f;
/* L10: */
...
/*        square roots. */
	...
	bsiz1 = (int) (sqrt((float) (qptr[curr + 1] - qptr[curr])) + .5f);
	bsiz2 = (int) (sqrt((float) (qptr[curr + 2] - qptr[curr + 1])) + 
		.5f);
	if (bsiz1 > 0) {
