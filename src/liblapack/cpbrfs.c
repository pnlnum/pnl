...
static int c__1 = 1;
 ...
 int cpbrfs_(char *uplo, int *n, int *kd, int *
	...
	complex *b, int *ldb, complex *x, int *ldx, float *ferr, float *
	berr, complex *work, float *rwork, int *info)
{
    ...
    int kase;
    float safe1, safe2;
    extern  int chbmv_(char *, int *, int *, complex *
...
, int *);
    ...
    float safmin;
    extern  int xerbla_(char *, int *), cpbtrs_(
...
/* Computing MAX */
		...
		for (i__ = MAX(i__3,i__4); i__ <= i__5; ++i__) {
		    ...
		    i__3 = l + i__ + k * ab_dim1;
		    i__4 = i__ + j * x_dim1;
		    s += ((r__1 = ab[i__3].r, ABS(r__1)) + (r__2 = r_imag(&
			    ab[l + i__ + k * ab_dim1]), ABS(r__2))) * ((r__3 
			    = x[i__4].r, ABS(r__3)) + (r__4 = r_imag(&x[i__ 
			    + j * x_dim1]), ABS(r__4)));
/* L40: */
		}
		i__5 = *kd + 1 + k * ab_dim1;
		rwork[k] = rwork[k] + (r__1 = ab[i__5].r, ABS(r__1)) * xk + 
			s;
/* L50: */
	    }
	} else {
