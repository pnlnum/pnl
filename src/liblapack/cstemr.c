...
static int c__1 = 1;
static float c_b18 = .003f;

 int cstemr_(char *jobz, char *range, int *n, float *d__, 
	float *e, float *vl, float *vu, int *il, int *iu, int *m, 
	float *w, complex *z__, int *ldz, int *nzc, int *isuppz, 
	int *tryrac, float *work, int *lwork, int *iwork, int *
	liwork, int *info)
{
    ...
    extern  int scopy_(int *, float *, int *, float *, 
	    int *);
    int wantz;
    extern  int slaev2_(float *, float *, float *, float *, float *
, float *, float *);
    int alleig;
    int ibegin;
    int indeig;
...
/*  of a float symmetric tridiagonal matrix T. Any such unreduced matrix has */
/*  a well defined set of pairwise different float eigenvalues, the corresponding */
/*  float eigenvectors are pairwise orthogonal. */
...
/*  float symmetric tridiagonal form. */

/*  (Any complex Hermitean tridiagonal matrix has float values on its diagonal */
/*  and potentially complex numbers on its off-diagonals. By applying a */
/*  similarity transform with an appropriate diagonal matrix */
/*  diag(1,e^{i \phy_1}, ... , e^{i \phy_{n-1}}), the complex Hermitean */
/*  matrix can be transformed into a float symmetric matrix and complex */
/*  arithmetic can be entirely avoided.) */

/*  While the eigenvectors of the float symmetric tridiagonal matrix are float, */
...
/*           WANTZ .EQ. FALSE. */
	...
	if (zquery && *info == 0) {
	    i__1 = z_dim1 + 1;
	    z__[i__1].r = (float) nzcmin, z__[i__1].i = 0.f;
