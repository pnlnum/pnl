...
static int c__1 = 1;
 ...
 int cgbrfs_(char *trans, int *n, int *kl, int *
	...
	ldx, float *ferr, float *berr, complex *work, float *rwork, int *
	info)
{
    ...
    int kase;
    float safe1, safe2;
    extern  int cgbmv_(char *, int *, int *, int *
...
, complex *, complex *, int *);
    ...
    float safmin;
    extern  int xerbla_(char *, int *), cgbtrs_(
	    char *, int *, int *, int *, int *, complex *, 
...
/* Computing MIN */
		...
		for (i__ = MAX(i__5,i__3); i__ <= i__4; ++i__) {
		    ...
		    i__3 = i__ + j * x_dim1;
		    s += ((r__1 = ab[i__5].r, ABS(r__1)) + (r__2 = r_imag(&
			    ab[kk + i__ + k * ab_dim1]), ABS(r__2))) * ((
			    r__3 = x[i__3].r, ABS(r__3)) + (r__4 = r_imag(&x[
			    i__ + j * x_dim1]), ABS(r__4)));
/* L60: */
		}
		rwork[k] += s;
/* L70: */
