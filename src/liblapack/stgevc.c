	...
	*work, int *info)
{
    ...
    float big;
    int lsa, lsb;
    float ulp, sum[4]	/* was [2][2] */;
    int ibeg, ieig, iend;
    float dmin__, temp, xmax, sump[4]	/* was [2][2] */, sums[4]	/* 
	    was [2][2] */, cim2a, cim2b, cre2a, cre2b;
    extern  int slag2_(float *, int *, float *, int *, 
	    float *, float *, float *, float *, float *, float *);
    float temp2, bdiag[2], acoef, scale;
    int ilall;
    int iside;
    float sbeta;
    ...
    extern  int sgemv_(char *, int *, int *, float *, 
	    float *, int *, float *, int *, float *, float *, int *), slaln2_(int *, int *, int *, float *, float *, 
	    float *, int *, float *, float *, float *, int *, float *, 
	    float *, float *, int *, float *, float *, int *);
    float temp2i, temp2r;
    int ilabad, ilbbad;
    float acoefa, bcoefa, cimaga, cimagb;
    int ilback;
    extern  int slabad_(float *, float *);
    float bcoefi, ascale, bscale, cfloata, cfloatb, bcoefr;
...
/* Computing MAX */
		r__3 = (r__1 = s[je + je * s_dim1], ABS(r__1)) * ascale, 
			r__4 = (r__2 = p[je + je * p_dim1], ABS(r__2)) * 
...
/*              Scale to avoid underflow */
		...
		if (lsb) {
/* Computing MAX */
		    r__1 = scale, r__2 = small / ABS(salfar) * MIN(bnorm,
			    big);
		    scale = MAX(r__1,r__2);
		}
		if (lsa || lsb) {
/* Computing MIN */
/* Computing MAX */
		    r__3 = 1.f, r__4 = ABS(acoef), r__3 = MAX(r__3,r__4), 
			    r__4 = ABS(bcoefr);
		    r__1 = scale, r__2 = 1.f / (safmin * MAX(r__3,r__4));
		    scale = MIN(r__1,r__2);
		    if (lsa) {
