...
static int c__1 = 1;
 ...
 int sgbrfsx_(char *trans, char *equed, int *n, int *
	kl, int *ku, int *nrhs, float *ab, int *ldab, float *afb, 
	int *ldafb, int *ipiv, float *r__, float *c__, float *b, int 
	*ldb, float *x, int *ldx, float *rcond, float *berr, int *
	n_err_bnds__, float *err_bnds_norm__, float *err_bnds_comp__, int *
	nparams, float *params, float *work, int *iwork, int *info)
{
    ...
    extern double sla_gbrcond__(char *, int *, int *, int *, 
	    float *, int *, float *, int *, int *, int *, float *
	    , int *, float *, int *, unsigned long);
    float cwise_wrong__;
    extern  int sla_gbrfsx_extended__(int *, int *, 
    ...
    float anorm;
    extern double slangb_(char *, int *, int *, int *, float *, 
	     int *, float *), slamch_(char *);
    extern  int sgbcon_(char *, int *, int *, int 
...
/*     Compute scaled normwise condition number cond(A*C). */
	...
	if (colequ && notran) {
	    rcond_tmp__ = sla_gbrcond__(trans, n, kl, ku, &ab[ab_offset], 
		    ...
		    info, &work[1], &iwork[1], (unsigned long)1);
	...
	for (j = 1; j <= i__1; ++j) {
		...
		rcond_tmp__ = sla_gbrcond__(trans, n, kl, ku, &ab[ab_offset], 
			ldab, &afb[afb_offset], ldafb, &ipiv[1], &c__1, &x[j *
			 x_dim1 + 1], info, &work[1], &iwork[1], (unsigned long)1);
	    } else {
		rcond_tmp__ = 0.f;
	    }
