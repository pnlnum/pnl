	...
	info)
{
    ...
    float s, v[3];
    int i1, i2;
    float t1, t2, t3, v2, v3, aa, ab, ba, bb, h11, h12, h21, h22, cs;
    int nh;
    float sn;
    int nr;
    float tr;
    ...
    float det, h21s;
    int its;
    float ulp, sum, tst, rt1i, rt2i, rt1r, rt2r;
    extern  int srot_(int *, float *, int *, float *, 
	    int *, float *, float *), scopy_(int *, float *, int *, 
	    float *, int *), slanv2_(float *, float *, float *, float *, float *
...
/*              ==== float shifts (use only one of them)  ==== */

		rt1r = tr + rtdisc;
		rt2r = tr - rtdisc;
		if ((r__1 = rt1r - h22, ABS(r__1)) <= (r__2 = rt2r - h22, 
			ABS(r__2))) {
		    rt1r *= s;
		    rt2r = rt1r;
		} else {
...
/*           overflows and most underflows.) */
	    ...
	    if ((r__1 = h__[m + (m - 1) * h_dim1], ABS(r__1)) * (ABS(v[1]) 
		    + ABS(v[2])) <= ulp * ABS(v[0]) * ((r__2 = h__[m - 1 + (
		    m - 1) * h_dim1], ABS(r__2)) + (r__3 = h__[m + m * 
		    h_dim1], ABS(r__3)) + (r__4 = h__[m + 1 + (m + 1) * 
		    h_dim1], ABS(r__4)))) {
		goto L60;
	    }
/* L50: */
