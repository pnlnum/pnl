	...
	wi)
{
    ...
    /* Local variables */
    float r__, c1, c2, c3, c4, c5, s1, s2, a11, a12, a21, a22, b11, b12, b22, 
...
/*          will always be positive.  If the eigenvalues are float, then */
/*          the first (float) eigenvalue is  WR1 / SCALE1 , but this may */
...
/*          eigenvalues are float, then the second (float) eigenvalue is */
/*          WR2 / SCALE2 , but this may overflow or underflow, and in */
/*          fact, SCALE2 may be zero or less than the underflow */
/*          threshhold if the exact eigenvalue is sufficiently large. */

/*  WR1     (output) REAL */
/*          If the eigenvalue is float, then WR1 is SCALE1 times the */
/*          eigenvalue closest to the (2,2) element of A B**(-1).  If the */
/*          eigenvalue is complex, then WR1=WR2 is SCALE1 times the float */
/*          part of the eigenvalues. */

/*  WR2     (output) REAL */
/*          If the eigenvalue is float, then WR2 is SCALE2 times the */
/*          other eigenvalue.  If the eigenvalue is complex, then */
/*          WR1=WR2 is SCALE1 times the float part of the eigenvalues. */

/*  WI      (output) REAL */
/*          If the eigenvalue is float, then WI is zero.  If the */
...
/* Computing MAX */
    r__5 = (r__1 = a[a_dim1 + 1], ABS(r__1)) + (r__2 = a[a_dim1 + 2], ABS(
	    r__2)), r__6 = (r__3 = a[(a_dim1 << 1) + 1], ABS(r__3)) + (r__4 =
	     a[(a_dim1 << 1) + 2], ABS(r__4)), r__5 = MAX(r__5,r__6);
...
/*     Perturb B if necessary to insure non-singularity */
    ...
    b22 = b[(b_dim1 << 1) + 2];
/* Computing MAX */
    r__1 = ABS(b11), r__2 = ABS(b12), r__1 = MAX(r__1,r__2), r__2 = ABS(
...
/* Computing MAX */
    r__1 = ABS(b11), r__2 = ABS(b12) + ABS(b22), r__1 = MAX(r__1,r__2);
...
/*        Choose (float) eigenvalue closest to 2,2 element of A*B**(-1) */
/*        for WR1. */
...
/* Computing MAX */
/* Computing MIN */
    r__3 = c4, r__4 = MAX(wabs,c5) * .5f;
    r__1 = MAX(*safmin,c1), r__2 = (wabs * c2 + c3) * 1.0000100000000001f, 
	    r__1 = MAX(r__1,r__2), r__2 = MIN(r__3,r__4);
    wsize = MAX(r__1,r__2);
    if (wsize != 1.f) {
	wscale = 1.f / wsize;
	if (wsize > 1.f) {
...
/*     Scale second eigenvalue (if float) */
...
/* Computing MAX */
	r__5 = ABS(*wr2);
	r__3 = c4, r__4 = MAX(r__5,c5) * .5f;
	r__1 = MAX(*safmin,c1), r__2 = (ABS(*wr2) * c2 + c3) * 
		1.0000100000000001f, r__1 = MAX(r__1,r__2), r__2 = MIN(r__3,
		r__4);
	wsize = MAX(r__1,r__2);
	if (wsize != 1.f) {
	    wscale = 1.f / wsize;
