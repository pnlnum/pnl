...
static int c_false = FALSE;
 ...
 int cgbrfsx_(char *trans, char *equed, int *n, int *
	kl, int *ku, int *nrhs, complex *ab, int *ldab, complex *
	afb, int *ldafb, int *ipiv, float *r__, float *c__, complex *b, 
	int *ldb, complex *x, int *ldx, float *rcond, float *berr, 
	int *n_err_bnds__, float *err_bnds_norm__, float *err_bnds_comp__, 
	int *nparams, float *params, complex *work, float *rwork, int *
	info)
{
    ...
    /* Local variables */
    float illrcond_thresh__, unstable_thresh__, err_lbnd__;
    int ref_type__;
    extern int ilatrans_(char *);
    int j;
    float rcond_tmp__;
    int prec_type__, trans_type__;
    float cwise_wrong__;
    extern  int cla_gbrfsx_extended__(int *, int *, 
	    int *, int *, int *, int *, complex *, int *, 
	    complex *, int *, int *, int *, float *, complex *, 
	    int *, complex *, int *, float *, int *, float *, float *
	    , complex *, float *, complex *, complex *, float *, int *, 
	    float *, float *, int *, int *);
    char norm[1];
    int ignore_cwise__;
    extern double cla_gbrcond_c__(char *, int *, int *, int *,
	     complex *, int *, complex *, int *, int *, float *, 
	    int *, int *, complex *, float *, unsigned long);
    extern int lsame_(char *, char *);
    float anorm;
    extern double cla_gbrcond_x__(char *, int *, int *, int *,
	     complex *, int *, complex *, int *, int *, complex *,
	     int *, complex *, float *, unsigned long), clangb_(char *, int *, 
	     int *, int *, complex *, int *, float *);
    extern  int cgbcon_(char *, int *, int *, int 
	    *, complex *, int *, int *, float *, float *, complex *, 
	    float *, int *);
    extern double slamch_(char *);
    extern  int xerbla_(char *, int *);
    int colequ, notran, rowequ;
    extern int ilaprec_(char *);
    int ithresh, n_norms__;
    float rthresh;
...
/*     Set default parameters. */
    ...
    illrcond_thresh__ = (float) (*n) * slamch_("Epsilon");
    ithresh = 10;
    rthresh = .5f;
    ...
    if (*nparams >= 2) {
	if (params[2] < 0.f) {
	    params[2] = (float) ithresh;
	} else {
	    ithresh = (int) params[2];
	}
    }
    if (*nparams >= 3) {
	if (params[3] < 0.f) {
	...
	if (colequ && notran) {
	    rcond_tmp__ = cla_gbrcond_c__(trans, n, kl, ku, &ab[ab_offset], 
		    ldab, &afb[afb_offset], ldafb, &ipiv[1], &c__[1], &c_true,
		     info, &work[1], &rwork[1], (unsigned long)1);
	    ...
	    rcond_tmp__ = cla_gbrcond_c__(trans, n, kl, ku, &ab[ab_offset], 
		    ...
		    c_false, info, &work[1], &rwork[1], (unsigned long)1);
	}
	i__1 = *nrhs;
	for (j = 1; j <= i__1; ++j) {
...
/*     cwise error is at least CWISE_WRONG. */
	...
	for (j = 1; j <= i__1; ++j) {
		...
		rcond_tmp__ = cla_gbrcond_x__(trans, n, kl, ku, &ab[ab_offset]
			...
			x_dim1 + 1], info, &work[1], &rwork[1], (unsigned long)1);
	    } else {
		rcond_tmp__ = 0.f;
	    }
