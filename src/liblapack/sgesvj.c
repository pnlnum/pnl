...
static float c_b17 = 0.f;
static float c_b18 = 1.f;
static int c__1 = 1;
static int c__0 = 0;
static int c__2 = 2;

 int sgesvj_(char *joba, char *jobu, char *jobv, int *m, 
	int *n, float *a, int *lda, float *sva, int *mv, float *v, 
	int *ldv, float *work, int *lwork, int *info)
{
    /* System generated locals */
    int a_dim1, a_offset, v_dim1, v_offset, i__1, i__2, i__3, i__4, i__5;
    float r__1, r__2;
    ...
    int ir1, jbc;
    float big;
    int kbl, igl, ibr, jgl, nbl;
    float tol;
    int mvl;
    float aapp, aapq, aaqq, ctol;
    int ierr;
    extern double sdot_(int *, float *, int *, float *, int *);
    float aapp0, temp1;
    extern double snrm2_(int *, float *, int *);
    float scale, large, apoaq, aqoap;
    ...
    int lsvec;
    float fastr[5];
    int applv, rsvec, uctol, lower, upper;
    extern  int scopy_(int *, float *, int *, float *, 
	    int *);
    int rotok;
    extern  int sswap_(int *, float *, int *, float *, 
	    int *), saxpy_(int *, float *, float *, int *, float *, 
	    int *), srotm_(int *, float *, int *, float *, int *
, float *), sgsvj0_(char *, int *, int *, float *, int *
, float *, float *, int *, float *, int *, float *, float *, 
	    float *, int *, float *, int *, int *), sgsvj1_(
...
, float *, int *, float *, int *, float *, float *, float *, 
    ...
    extern  int slassq_(int *, float *, int *, float *, 
	    float *);
    float mxsinj;
    int emptsw, notrot, iswrot, lkahead;
    int goscale, noscale;
    float rootbig, epsilon, rooteps;
    int rowskip;
    float roottol;
...
/*  Purpose */
/*  ~~~~~~~ */
/*  SGESVJ computes the singular value decomposition (SVD) of a float */
...
/*     in A are detected, the procedure returns with INFO=-6. */
    ...
    scale = 1.f / sqrt((float) (*m) * (float) (*n));
    noscale = TRUE;
    goscale = TRUE;
...
/* Computing MIN */
	r__1 = sn / aaqq, r__2 = big / (aapp * sqrt((float) (*n)));
	temp1 = MIN(r__1,r__2);
/*         AAQQ  = AAQQ*TEMP1 */
/*         AAPP  = AAPP*TEMP1 */
    } else if (aaqq >= sn && aapp >= temp1) {
...
/* Computing MIN */
	r__1 = sn / aaqq, r__2 = big / (sqrt((float) (*n)) * aapp);
	temp1 = MIN(r__1,r__2);
/*         AAQQ  = AAQQ*TEMP1 */
/*         AAPP  = AAPP*TEMP1 */
    } else {
...
/*     Additional steering devices */
	...
	if (i__ > swband + 1 && mxaapq < sqrt((float) (*n)) * tol && (float) (*
		n) * mxaapq * mxsinj < tol) {
	    goto L1994;
	}
