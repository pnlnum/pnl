...
static int c_n1 = -1;
 ...
 int cheevx_(char *jobz, char *range, char *uplo, int *n, 
	...
	complex *work, int *lwork, float *rwork, int *iwork, int *
	ifail, int *info)
{
    ...
    extern  int scopy_(int *, float *, int *, float *, 
	    int *);
    int wantz;
    extern double clanhe_(char *, char *, int *, complex *, int *, 
	     float *);
    int alleig, indeig;
    int iscale, indibl;
    int valeig;
...
/*     Test the input parameters. */
	...
	*info = -6;
	...
	if (*n <= 1) {
	    lwkmin = 1;
	    work[1].r = (float) lwkmin, work[1].i = 0.f;
	} else {
	    lwkmin = *n << 1;
