...
\begin{itemize}
\item \describefun{void}{pnl_cdf_bet}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$y, double $\ast$a, double $\ast$b, 
    int $\ast$status, double $\ast$bound}
  \sshortdescribe Cumulative Distribution Function BETA distribution.  

\item \describefun{void}{pnl_cdf_bin}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$xn, double $\ast$pr, double
    $\ast$ompr, int $\ast$status, double $\ast$bound}
  \sshortdescribe Cumulative Distribution Function BINa distribution.

\item \describefun{void}{pnl_cdf_chi}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$df, int $\ast$status, double
    $\ast$bound}
  \sshortdescribe Cumulative Distribution Function CHI-Square distribution.  

\item \describefun{void}{pnl_cdf_chn}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$df, double $\ast$pnonc, int
    $\ast$status, double $\ast$bound}
  \sshortdescribe Cumulative Distribution Function Non-central Chi-Square distribution.  

\item \describefun{void}{pnl_cdf_f}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$dfn, double $\ast$dfd, int
    $\ast$status, double $\ast$bound}
  \sshortdescribe Cumulative Distribution Function F distribution.  

\item \describefun{void}{pnl_cdf_fnc}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$dfn, double $\ast$dfd, double
    $\ast$pnonc, int $\ast$status, double $\ast$bound}
  \sshortdescribe Cumulative Distribution Function Non-central F distribution.  

\item \describefun{void}{pnl_cdf_gam}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$shape, double $\ast$scale, int
    $\ast$status, double $\ast$bound}
  \sshortdescribe Cumulative Distribution Function GAMma distribution.  

\item \describefun{void}{pnl_cdf_nbn}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$xn, double $\ast$pr, double
    $\ast$ompr, int $\ast$status, double $\ast$bound}
  \sshortdescribe Cumulative Distribution Function Negative BiNomial distribution.  

\item \describefun{void}{pnl_cdf_nor}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$mean, double $\ast$sd, int
    $\ast$status, double $\ast$bound}
  \sshortdescribe Cumulative Distribution Function NORmal distribution.  

\item \describefun{void}{pnl_cdf_poi}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$xlam, int $\ast$status, double
    $\ast$bound}
  \sshortdescribe Cumulative Distribution Function POIsson distribution.  

\item \describefun{void}{pnl_cdf_t}{int $\ast$which, double $\ast$p, double
    $\ast$q, double $\ast$x, double $\ast$df, int $\ast$status, double
    $\ast$bound}
...
\item \describefun{void}{pnl_cdfbchi2n}{double x, double df, double ncparam, double
  ...
  of the random variable \var{beta $\ast$X} where \var{X} is non central $\chi^2$
...
generator using directly the value of the macro \var{PNL_RNG_XXX} rather than
...
\item \describefun{int}{pnl_rand_or_quasi}{int generator}
  \sshortdescribe Returns the type the generator of index \var{generator}, 
  \var{MC} or \var{QMC}
  
  
\item \describefun{const char $\ast$}{pnl_rand_name}{int type_generator}
  \sshortdescribe Returns the name of the generator of index \var{generator}
\end{itemize}
...
\begin{itemize}
\item \describefun{int}{pnl_rand_bernoulli}{double p, int generator}
...
The following functions take an already existing \refstruct{PnlVect}$\ast$ as
...
number generator.
\begin{itemize}
\item \describefun{void}{pnl_vect_rand_uni}{\refstruct{PnlVect} $\ast$G, int
...
\item \describefun{void}{pnl_vect_rand_normal}{\refstruct{PnlVect} $\ast$G, 
    int samples, int generator}
  \sshortdescribe \var{G} is a vector of independent and identically distributed
  samples from the standard normal distribution.

\item \describefun{void}{pnl_vect_rand_uni_d}{\refstruct{PnlVect} $\ast$G, int
    d, double a, double b, int type_generator}
  \sshortdescribe \var{G} is a sample from the uniform distribution on $[a, 
  b]^{\text{d}}$.

\item \describefun{void}{pnl_vect_rand_normal_d}{\refstruct{PnlVect} $\ast$G, 
    int d, int generator}
...
The following functions take an already existing \refstruct{PnlMat}$\ast$ as
first argument and fill each entry of the vector with a sample from the
...
\begin{itemize}
\item \describefun{void}{pnl_mat_rand_uni}{\refstruct{PnlMat} $\ast$M, int
    samples, int d, const PnlVect $\ast$a, const PnlVect $\ast$b, int
    type_generator}
  \sshortdescribe \var{M} contains \var{samples} samples from the uniform
  distribution on $\prod_{i=1}^d [a_i, b_i]$.

\item \describefun{void}{pnl_mat_rand_uni2}{\refstruct{PnlMat} $\ast$M, int
    samples, int d, double a, double b, int type_generator}
  \sshortdescribe \var{M} contains \var{samples} samples from the uniform
  distribution on $[a, b]^{\text{d}}$.
  
\item \describefun{void}{pnl_mat_rand_normal}{\refstruct{PnlMat} $\ast$M, int
