...
\item \describefun{void}{pnl_vect_find}{\refstruct{PnlVectInt} \ptr
  ind, \refstruct{PnlVect} \ptr val, const \refstruct{PnlVect} \ptr V,
  int(\ptr f)(double)}
  \sshortdescribe \var{ind} contains on exit the indices \var{i} for which
  \var{f(V(i)) ==1 }. If \var{val} is not NULL, on exit \var{val = V(ind)}.

\item \describefun{void}{pnl_vect_find_vect}{\refstruct{PnlVectInt} \ptr
  ind, \refstruct{PnlVect} \ptr val, const \refstruct{PnlVect} \ptr V1, 
  const \refstruct{PnlVect} \ptr V2, int(\ptr f)(double, double)}
  \sshortdescribe \var{ind} contains on exit the indices \var{i} for which
  \var{f(V1(i),V2(i)) ==1 }. If \var{val} is not NULL, on exit \var{val = V(ind)}.
\end{itemize}
...
\item \describefun{void}{pnl_mat_find}{\refstruct{PnlVectInt} \ptr
  indi, \refstruct{PnlVectInt} \ptr indj, \refstruct{PnlVect} \ptr val,
  const \refstruct{PnlMat} \ptr M, int(\ptr f)(double)}
  \sshortdescribe If \var{indj} is not NULL, \var{indi} and \var{indj} contains
  on exit the indices \var{(i,j)} for which \var{f(M(i,j))==1}. If \var{indj}
  is NULL, then a linear indexing is used : \var{M} is seen as a large vector
  built by stacking the rows of \var{M} and \var{indi} contains the values of
  the linear index for which the test is positive. If \var{val} is not NULL, it
  is used to store {\bf in a vector} the entries of \var{M} for which the test
  \var{f} is positive, i.e. \var{val = M((indi(:), indj(:)))}.

\item \describefun{void}{pnl_mat_find_mat}{\refstruct{PnlVectInt} \ptr
  indi, \refstruct{PnlVectInt} \ptr indj,  \refstruct{PnlVect} \ptr val,
  const \refstruct{PnlMat} \ptr M1,
  const \refstruct{PnlMat} \ptr M2, int(\ptr f)(double, double)}
  \sshortdescribe If \var{indj} is not NULL, \var{indi} and \var{indj} contains
  on exit the indices \var{(i,j)} for which \var{f(M1(i,j), M2(i,j))==1}. If
  \var{indj} is NULL, then a linear indexing is used : \var{M1} and \var{M2} are
  seen as large vectors built by stacking the rows of the matrices and
  \var{indi} contains the values of the linear index for which the test is
  positive. If \var{val} is not NULL, it is used to store {\bf in a vector} the
  entries of \var{M1} for which the test \var{f} is positive, i.e. \var{val =
  M((indi(:), indj(:)))}.
\end{itemize}
...
\begin{itemize}
  \item 
    \describefun{\refstruct{PnlTridiagMat} \ptr }{pnl_tridiag_mat_create}{int size}
...
\begin{itemize}
  \item \describefun{void}{pnl_tridiag_mat_mult_vect_inplace}{\refstruct{PnlVect} \ptr lhs, const \refstruct{PnlTridiagMat} \ptr mat, const \refstruct{PnlVect} \ptr rhs}
    \sshortdescribe In place matrix multiplication  
  \item \describefun{\refstruct{PnlVect} \ptr }{pnl_tridiag_mat_mult_vect}{const \refstruct{PnlTridiagMat} \ptr mat, const \refstruct{PnlVect} \ptr vec}
  ...
  \item \describefun{void}{pnl_tridiag_mat_lAxpby}{double lambda, const \refstruct{PnlTridiagMat}
    ...
    content of \var{b} is not used on input and instead \var{b} is resized to
    match \var{A*x}
  \item \describefun{double}{pnl_tridiag_mat_scalar_prod}{const \refstruct{PnlVect} \ptr x,const \refstruct{PnlTridiagMat} \ptr A, const \refstruct{PnlVect} \ptr y}
    \sshortdescribe Computes \var{x' * A * y}
