...
It is possible to create Mersenne Twister random number generator each with
...
\item \describefun{void}{pnl_mt_sseed}{mt_state \ptr state, unsigned long int
    s}
  \describefun Sets the initial value of variable \var{state} using \var{s}
\item \describefun{ulong}{pnl_mt_genrand}{mt_state \ptr state}
  \describefun Returns the following number in the sequence as an unsigned
...
\item \describefun{double}{pnl_mt_genrand_double}{mt_state \ptr state}
  \describefun Returns the following number in the sequence as a double.
\end{itemize}
...
\item \describefun{\refstruct{PnlRng} \ptr \ptr}{pnl_rng_dcmt_create_array}{int n, ulong seed, int \ptr count}
  ...
  directly returns an array of \reffun{PnlRng}.
