...
The following functions return one sample from a specified law.
\begin{itemize}
\item \describefun{int}{pnl_rng_bernoulli}{double p, PnlRng *rng}
  \sshortdescribe Generates a sample from the Bernouilli law on $\{0, 1\}$ with
  parameter \var{p}.
  
\item \describefun{long}{pnl_rng_poisson}{double lambda, PnlRng *rng}
  \sshortdescribe Generates a sample from the Poisson law with
  parameter \var{lambda}.

\item \describefun{double}{pnl_rng_exp}{double lambda, PnlRng *rng}
  \sshortdescribe Generates a sample from the Exponential law with
  parameter \var{lambda}.

\item \describefun{double}{pnl_rng_uni} {PnlRng *rng}
  \sshortdescribe Generates a sample from the Uniform law on $[0, 1]$.
...
\item \describefun{double}{pnl_rng_normal} {PnlRng *rng}
  \sshortdescribe Generates a sample from the standard normal distribution.
...
\item \describefun{double}{pnl_rng_gamma} {double a, double b, PnlRng *rng}
  \sshortdescribe Generates a sample from the $\Gamma(a, b)$ distribution.  
  
\item \describefun{double}{pnl_rng_chi2} {double n, PnlRng *rng}
  \sshortdescribe Generates a sample from the centered $\chi^2(n)$ distribution.
\end{itemize}
...
Besides the generators listed in Table~\ref{rng_indices}, it is possible to
add some more generators. For the moment, only ``Dynamically created Mersenne
Twister'' random generators (DCMT) can be added to the list of dynamic random number
generators. Once a generator is added to this list, it is assigned an ID which
makes it possible to use it through all the functions listed above.

Some functions to create a DCMT random number generator.
\begin{itemize}
...
\item \describefun{dcmt_state \ptr \ptr}{pnl_dcmt_create_array}{int n, ulong seed, int \ptr count}
  ...
  created.
\item \describefun{\refstruct{PnlRng} \ptr \ptr}{pnl_rng_dcmt_create_array}{int n, ulong seed, int \ptr count}
  \sshortdescribe Creates an array of \var{n} independent DCMT. \var{seed} is
  the seed used to initialize the Mersenne Twister generator internally used to
  find new DCMT. On exit, \var{count} contains the number of generators actually
...
Some functions to manipulate the list of dynamic rng
\begin{itemize}
\item \describefun{int}{pnl_rand_add}{PnlRng \ptr rng}
  \sshortdescribe Adds the generator \var{rng} to the list of dynamic
  rng. This function returns an integer which can be used as a
  \var{type_generator} argument for the functions of Section~\ref{std-rng}
\item \describefun{int}{pnl_rand_add_dcmt}{dcmt_state \ptr mts}
  \sshortdescribe Adds the generator \var{mts} to the list of dynamic
  rng. This function returns an integer which can be used as a
  \var{type_generator} argument for the functions of Section~\ref{std-rng}
\item \describefun{void}{pnl_rand_remove}{int id}
  \sshortdescribe Removes a dcmt from the list of dynamic rng. \var{id} must
  be an index previously returned by \reffun{pnl_rand_add_dcmt}.
\end{itemize}
