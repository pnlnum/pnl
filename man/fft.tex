...
\item \describefun{int}{pnl_fft2d_inplace}{\refstruct{PnlMatComplex} \ptr data}
  \sshortdescribe Computes the 2D FFT of \var{data}. This function applies a 1D
  FFT to each row of the matrix and then a 1D FFT to each column of the modified
  matrix.
\item \describefun{int}{pnl_ifft2d_inplace}{\refstruct{PnlMatComplex} \ptr data}
  \sshortdescribe Computes the inverse 2D FFT of \var{data}. This function is
  the inverse of the function \reffun{pnl_fft_2d_inplace}.
\item \describefun{int}{pnl_fft2d}{const \refstruct{PnlMatComplex} \ptr in, \refstruct{PnlMatComplex} \ptr out}
  \sshortdescribe Computes the 2D FFT of \var{in} and stores it into \var{out}.
\item \describefun{int}{pnl_ifft2d}{const \refstruct{PnlMatComplex} \ptr in, \refstruct{PnlMatComplex} \ptr out}
  \sshortdescribe Computes the inverse 2D FFT of \var{in} and stores it into \var{out}.
\item \describefun{int}{pnl_real_fft2d}{const \refstruct{PnlMat} \ptr in, \refstruct{PnlMatComplex} \ptr out}
  \sshortdescribe Computes the 2D FFT of the real matrix \var{in} and stores it into \var{out}.
\item \describefun{int}{pnl_real_ifft2d}{const \refstruct{PnlMatComplex} \ptr in, \refstruct{PnlMatComplex} \ptr out}
  \sshortdescribe Computes the inverse 2D FFT of the complex matrix \var{in}
