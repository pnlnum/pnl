...
\begin{itemize}
  \item \describefun{\refstruct{PnlBasis} *}{pnl_basis_init}{int index, int N, int d}
...
\item \describefun{int}{pnl_fit_least_squares}{\refstruct{PnlVect}\ptr  coef, 
    \refstruct{PnlMat}\ptr  x, \refstruct{PnlVect}\ptr  y, 
    \refstruct{PnlBasis}\ptr  P, int N}
...
\item \describefun{double}{pnl_basis_eval}{\refstruct{PnlVect}\ptr  coef, double
    \ptr x, \refstruct{PnlBasis}\ptr  *P}
  ...
  \reffun{pnl_fit_least_squares}, this function returns $\sum_{k=0}^n
  \var{coef}_k  P_k(\var{x})$ where \var{x} is a C array.

\item \describefun{double}{pnl_basis_eval_D}{\refstruct{PnlVect}\ptr  coef, double
    \ptr x, \refstruct{PnlBasis}\ptr  *P, int i}
  ...
  coefficients computed by the function \reffun{pnl_fit_least_squares}, this
  function returns $\partial_{x_i} \sum_{k=0}^n \var{coef}_k  P_k(\var{x})$
  where \var{x} is a C array.

\item \describefun{double}{pnl_basis_eval_D2}{\refstruct{PnlVect}\ptr  coef, double
    \ptr x, \refstruct{PnlBasis}\ptr  *P, int i, int j}
  \sshortdescribe Computes the derivative with repsect to \var{x_i} of the
  linear combination of \var{P_k(x)} defined by \var{coef}. Given the
  coefficients computed by the function \reffun{pnl_fit_least_squares}, this
