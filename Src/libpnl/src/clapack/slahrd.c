...
/*           Update A(1:n,i)   
	    ...
	    i__2 = i__ - 1;
	    sgemv_("No transpose", n, &i__2, &c_b4, &y[y_offset], ldy, &a_ref(
...
/*           Apply I - V * T' * V' to this column (call it b) from the   
	    ...
	    i__2 = i__ - 1;
	    scopy_(&i__2, &a_ref(*k + 1, i__), &c__1, &t_ref(1, *nb), &c__1);
	    i__2 = i__ - 1;
	    strmv_("Lower", "Transpose", "Unit", &i__2, &a_ref(*k + 1, 1), 
		    lda, &t_ref(1, *nb), &c__1);
...
/*           w := w + V2'*b2 */
	    ...
	    i__2 = *n - *k - i__ + 1;
	    i__3 = i__ - 1;
	    sgemv_("Transpose", &i__2, &i__3, &c_b5, &a_ref(*k + i__, 1), lda,
...
/*           b2 := b2 - V2*w */
	    ...
	    i__2 = *n - *k - i__ + 1;
	    i__3 = i__ - 1;
	    sgemv_("No transpose", &i__2, &i__3, &c_b4, &a_ref(*k + i__, 1), 
...
/*           b1 := b1 - V1*w */
	    ...
	    i__2 = i__ - 1;
	    strmv_("Lower", "No transpose", "Unit", &i__2, &a_ref(*k + 1, 1), 
		    lda, &t_ref(1, *nb), &c__1);
	    i__2 = i__ - 1;
	    saxpy_(&i__2, &c_b4, &t_ref(1, *nb), &c__1, &a_ref(*k + 1, i__), &
		    c__1);
...
/*        Generate the elementary reflector H(i) to annihilate   
   ...
   Computing MIN */
	i__2 = *k + i__ + 1;
	i__3 = *n - *k - i__ + 1;
	slarfg_(&i__3, &a_ref(*k + i__, i__), &a_ref(min(i__2,*n), i__), &
		c__1, &tau[i__]);
	ei = a_ref(*k + i__, i__);
	a_ref(*k + i__, i__) = 1.f;
...
/*        Compute  Y(1:n,i) */
	...
	i__2 = *n - *k - i__ + 1;
	sgemv_("No transpose", n, &i__2, &c_b5, &a_ref(1, i__ + 1), lda, &
	...
	i__2 = i__ - 1;
	sgemv_("No transpose", n, &i__2, &c_b4, &y[y_offset], ldy, &t_ref(1, 
		i__), &c__1, &c_b5, &y_ref(1, i__), &c__1);
	sscal_(n, &tau[i__], &y_ref(1, i__), &c__1);

/*        Compute T(1:i,i) */

	i__2 = i__ - 1;
	r__1 = -tau[i__];
	sscal_(&i__2, &r__1, &t_ref(1, i__), &c__1);
	i__2 = i__ - 1;
