...
/*        Loop until stopping criterion is satisfied.   
	...
	if (upper) {
		...
		rwork[1] = (r__1 = bi.r, dabs(r__1)) + (r__2 = r_imag(&bi), 
			...
			= x[i__2].r, dabs(r__7)) + (r__8 = r_imag(&x_ref(2, j)
			), dabs(r__8)));
		i__2 = *n - 1;
		for (i__ = 2; i__ <= i__2; ++i__) {
		    ...
		    rwork[i__] = (r__1 = bi.r, dabs(r__1)) + (r__2 = r_imag(&
			    ...
			    x_ref(i__ - 1, j)), dabs(r__6))) + ((r__7 = dx.r, 
			    dabs(r__7)) + (r__8 = r_imag(&dx), dabs(r__8))) + 
			    ((r__9 = e[i__5].r, dabs(r__9)) + (r__10 = r_imag(
			    &e[i__]), dabs(r__10))) * ((r__11 = x[i__6].r, 
			    dabs(r__11)) + (r__12 = r_imag(&x_ref(i__ + 1, j))
			    , dabs(r__12)));
/* L30: */
		}
		i__2 = b_subscr(*n, j);
		...
		rwork[*n] = (r__1 = bi.r, dabs(r__1)) + (r__2 = r_imag(&bi), 
			...
			.r, dabs(r__5)) + (r__6 = r_imag(&x_ref(*n - 1, j)), 
			dabs(r__6))) + ((r__7 = dx.r, dabs(r__7)) + (r__8 = 
			r_imag(&dx), dabs(r__8)));
	    }
	} else {
	    if (*n == 1) {
...
/*           Update solution and try again. */
	    ...
	    caxpy_(n, &c_b16, &work[1], &c__1, &x_ref(1, j), &c__1);
	    lstres = berr[j];
	    ++count;
	    goto L20;
	}
...
/*        Normalize error. */
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
/* Computing MAX */
	    r__1 = lstres, r__2 = c_abs(&x_ref(i__, j));
